    ...
    headings.forEach(h3 => {
        ...
        if (code) {
            ...
            while (next && next.tagName !== 'H3' && next.tagName !== 'H2') {
                ...
                if (next.tagName === 'TABLE') {
                    ...
                    rows.forEach(row => {
                        ...
                        if (cols.length >= 7) {
                            const num = parseInt(cols[0].innerText) || 0;
                            const pos = cols[1].innerText.trim().split(' ').pop();
                            const pName = cols[2].innerText.split('(')[0].trim();
                            const is_cap = cols[2].innerText.includes('(captain)');
            ...
            if (players.length > 0) {
                ...
                if (selected.length < 11) selected = selected.concat(players.filter(p => !selected.includes(p)).slice(0, 11 - selected.length));
                if (selected.length < 15) selected = selected.concat(players.filter(p => !selected.includes(p)).slice(0, 15 - selected.length));
